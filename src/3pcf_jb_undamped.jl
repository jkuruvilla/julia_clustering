...
function jingborner_connected(r1::Float64, r2::Float64, coschi::Float64, xi, eta_zero_integrand, eta_two_integrand, epsilon_two_integrand)
    first_term  = 10.0/7.0 * xi(r1) * xi(r2)
    second_term = - (eta_two_integrand(r1) * eta_zero_integrand(r2) * coschi *r1 * r2)
    third_term  = - (eta_zero_integrand(r1) * eta_two_integrand(r2) * coschi *r1 * r2)
    fourth_term = 4.0/7.0 * epsilon_two_integrand(r1) * epsilon_two_integrand(r2) * coschi^2 * r1^2 * r2^2
    fifth_term  = 4.0/7.0 * epsilon_two_integrand(r1) * eta_two_integrand(r2) * r1^2
    sixth_term  = 4.0/7.0 * eta_two_integrand(r1) * epsilon_two_integrand(r2) * r2^2
    seven_term  = 4.0/7.0 * 3.0 * eta_two_integrand(r1) * eta_two_integrand(r2)
...
open("../results/Euclid/linear_correlation_julia_flagship_undamped.txt", "w") do io
    writedlm(io, [_rrr, corr])
end

open("../results/Euclid/eta_zero_julia_flagship_undamped.txt", "w") do io
    writedlm(io, [_rrr, eta_zero])
end
open("../results/Euclid/eta_two_julia_flagship_undamped.txt", "w") do io
...
# end
open("../results/Euclid/epsilon_two_julia_flagship_undamped.txt", "w") do io
...
open("../results/Euclid/jb_julia_flagship_undamped.txt", "w") do io
